...
export function validateCurp(curp: string): CurpValidationResult {
  ...
  if (regex.test(curpNorm)) {
    ...
    if (PALABRAS_OFENSIVAS.includes(primeras4)) {
      reasons.push(
        "Las primeras cuatro letras forman una palabra no permitida en una CURP oficial."
      );
    }
    ...
    if (VALIDAR_DIGITO_VERIFICADOR) {
      ...
      if (digitoReal !== String(digitoCalculado)) {
        reasons.push(
          `El dígito verificador de la CURP no es válido. Se esperaba ${digitoCalculado}.`
        );
      }
    }
  }
